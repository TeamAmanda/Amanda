...
from Amanda.modules.helper_funcs.alternate import typing_action, send_action
...
def gtts(update: Update, context: CallbackContext):
    ...
    try:
        tts = gTTS(reply)
        tts.save("Amanda.mp3")
        with open("Amanda.mp3", "rb") as speech:
            msg.reply_audio(speech)
    finally:
        if os.path.isfile("Amanda.mp3"):
            os.remove("Amanda.mp3")
